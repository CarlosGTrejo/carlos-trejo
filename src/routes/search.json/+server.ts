...
import type { SearchablePost, PostType, Categories } from '$lib/types';
...
async function getAllContent(): Promise<SearchablePost[]> {
    ...
    const content = await Promise.all(
        Object.entries(rawPaths).map(async ([path, rawContent]) => {
            ...
            return {
                ...
                date: metadata?.date || '',
                categories: (metadata?.categories as Categories[]) || [],
                content: plainTextContent,
                type
            } satisfies SearchablePost;
        })
    );

    // Filter out null entries and items without titles, then sort
    return content
        .filter((item): item is SearchablePost =>
            item !== null && Boolean(item.title && item.slug)
        )
        .sort((a, b) => {
            // Sort by type (articles first), then by date (newest first)
            if (a.type !== b.type) {
                const typeOrder: Record<PostType, number> = { article: 0, note: 1, find: 2 };
